...
`provenance.json` records three separate responsibilities:
...
| Production deployment and secret references | `ORESoftware/k8s-cluster` | Repository identity; deployment revisions remain GitOps-owned |

CI runs `npm run provenance:check` and fails when these fields become mutable, unknown fields are
introduced, the source and production runtime are conflated, or the six Slack command aliases drift
from `config/commands.json`.
...
Production Slack ingress, request-signature enforcement, and project-aware dispatch remain owned by
the pinned Rust runtime. Production Kubernetes configuration, Argo CD reconciliation, workload
identity, and secret references remain owned by `ORESoftware/k8s-cluster`.
...
1. Update application or manifest behavior in a feature branch.
2. Update tests and regenerate `manifest.json` when command configuration changes.
3. Change `provenance.json` only as a deliberate review item with immutable replacement commits.
...
7. Record the merge commit, workflow run, and deployment revision in Linear and the owning GitHub
   Project item.
